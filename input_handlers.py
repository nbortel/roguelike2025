...
class PopupMessage(BaseEventHandler):
    ...
    def ev_keydown(
            self, event: tcod.event.KeyDown) -> Optional[BaseEventHandler]:
...
class AskUserEventHandler(EventHandler):
    ...
    def on_exit(self) -> Optional[ActionOrHandler]:
        ...
        By Default this returns to the main game event handler.
        """
...
class InventoryEventHandler(AskUserEventHandler):
    ...
    def on_render(self, console: tcod.Console) -> None:
        ...
        Will moved to a different position based on where the player is
...
class InventoryActivateHandler(InventoryEventHandler):
    ...
    def on_item_selected(self, item: Item) -> Optional[ActionOrHandler]:
        if item.consumable:
            # Return thje action for the selected item.
...
class SelectIndexHandler(AskUserEventHandler):
    ...
    def ev_keydown(
            self, event: tcod.event.KeyDown) -> Optional[ActionOrHandler]:
        """Check for key movement or confrimation keys."""
        key = event.sym
        if key in MOVE_KEYS:
            ...
            if event.mod & (
                ...
                modifier *= 5
            if event.mod & (
                    tcod.event.Modifier.LCTRL | tcod.event.Modifier.RCTRL):
                modifier *= 10
            if event.mod & (
                    tcod.event.Modifier.LALT | tcod.event.Modifier.RALT):
                modifier *= 20
            ...
            x += dx * modifier
            y += dy * modifier
            # Clamp the cursor index to the max size
    ...
    def on_index_selected(
            self, x: int, y: int) -> Optional[ActionOrHandler]:
...
class SingleRangedAttackHandler(SelectIndexHandler):
    """Handles targeting a single enemy. Only the enemy selected will be
    affected."""
...
class AreaRangedAttackHandler(SelectIndexHandler):
    ...
    def on_render(self, console: tcod.Console) -> None:
        ...
        affected tiles."""
        console.draw_frame(
                x=x - self.radius - 1, y=y - self.radius - 1,
...
class HistoryViewer(EventHandler):
    ...
    def on_render(self, console: tcod.Console) -> None:
        ...
        log_console.print_box(
                0, 0, log_console.width, 1,
                "|Message history|", alignment=libtcodpy.CENTER
        )

        # Render te message log using the cursor parameter
    ...
    def ev_keydown(
            self, event: tcod.event.KeyDown) -> Optional[MainGameEventHandler]:
